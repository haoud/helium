use crate::{cpu::Privilege, instruction, tss::TaskStateSegment};
...
pub static GDT: Spinlock<Table<{ 6 + crate::MAX_CPUS * 2 }>> = Spinlock::new(Table::empty());
...
pub unsafe fn setup() {
    ...
    GDT.lock().set_descriptor(3, &Descriptor::USER_CODE64);
    GDT.lock().set_descriptor(4, &Descriptor::USER_DATA);
    load();
}
