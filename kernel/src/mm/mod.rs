...
pub static FRAME_ALLOCATOR: Lazy<Spinlock<dummy::Allocator>> =
    Lazy::new(|| {
...
/// Initializes the memory subsystem. It parses the memory map and fills the
/// frame array in order to allow the allocation of physical memory frames,
/// and then allocate a range of frames for the heap.
///
/// # Safety
/// This function is unsafe because it use unsafe code and raw pointers to
/// initialize the memory subsystem.
///
/// # Panics
/// Panics if there is not enough memory to allocate the heap. This can happen
/// if the kernel is ran on a very constrained system.
#[init]
pub unsafe fn setup() {
...
/// during the boot process. Trying to use the memory after calling this
/// function will cause undefined behavior: the caller must ensure that there
/// is no more references to the memory that will be freed.
pub unsafe fn reclaim_boot_memory() {
    ...
    let mut size = FRAME_ALLOCATOR
        ...
        .iter()
        .map(|range| {
            range.end.addr().as_usize() - range.start.addr().as_usize()
        })
        .sum::<usize>();
