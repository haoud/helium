...
/// Represent an UTF-8 string that is stored in the userland address space. This structure is
...
impl UserString {
    ...
    /// Creates a new user string from a user pointer to an userpace [`SyscallString`]. If the
    /// pointer contained in the syscall string is invalid, this function will return `None`.
    #[must_use]
    ...
    /// the userland address space to the kernel address space and return it as an `String`. All
    /// modifications to the returned string will not affect the userland string.
    ///
    /// # Errors
    /// This function will return an error if any of the following conditions are met (see the
    /// [`FetchError`] enum for more details):
    /// - The user pointer is invalid: not mapped, not readable or not in the userland address
    ...
    pub fn fetch(&self) -> Result<String, FetchError> {
        ...
        // SAFETY: This is safe because we checked that the string is entirely in the userland
        // address space and that the string is not too long to be handled by the kernel.
        // Data race are permitted here because the string resides in the userland address space
        // and the kernel cannot prevent data races in the userland address space: it is the
