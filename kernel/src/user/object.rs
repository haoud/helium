...
/// that holds an pointer to the object in the userland address space and an
/// copy of the object in the kernel address space. When the `Object` struct
/// is dropped, it will write the object back to the userland address space,
/// so the object in the userland address space will be updated.
///
/// If you only need to read once an object from the userland address space
/// without updating it, then you should use the [`read`] function of this
/// struct instead of creating an `Object` struct, as it will be more efficient
/// when the object will be dropped.
#[derive(Debug)]
pub struct Object<T> {
...
impl<T> Object<T> {
    ...
    /// memory. This function will read the object from the userland memory
    /// and store it in the `Object` struct. When the `Object` struct is
    /// dropped, it will write the object back to the userland memory to update
    /// the object in the userland memory if any changes were made.
    ///
    /// # Safety
    ...
    /// userland memory will be updated. This function is automatically called
    /// when the `Object` struct is dropped, so you do not need to call this
    /// function manually in most cases.
    ///
    /// # Safety
    /// This function is unsafe because it dereference a raw user pointer and
    /// use the `copy_from` function to copy the object from the userland
    ...
    /// if you does not need to modify the object in the userland memory.
    ///
    /// # Safety
    /// This function is unsafe because it dereference a raw user pointer and
    /// use the `copy_from` function to copy the object from the userland
    ...
    /// the `Object` struct if you does not need to read the object from the
...
impl<T> Drop for Object<T> {
    /// Write the object back to the userland memory to update the object in
    /// the userland memory when the `Object` struct is dropped.
    fn drop(&mut self) {
        unsafe {
            self.update();
        }
