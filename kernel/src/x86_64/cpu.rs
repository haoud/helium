...
/// The interrupt frame pushed by the CPU and by the interrupt stubs when
...
/// The different privilege levels of the CPU. In the kernel, we only use
/// Ring 0 and Ring 3, respectively for kernel and user code.
...
bitflags! {
    ...
    pub struct Cr4: u64 {
        ...
        /// Prevent the execution of the SGDT, SIDT, SLDT, SMSW, and STR
        /// instructions in user mode software.
        const UMPI = 1 << 11;
        ...
        /// Enable user software to read and write their own FS and GS segment
        /// base
        const FSGSBASE = 1 << 16;
        ...
        /// Enable extended processor state management instructions, including
        /// XSAVE, XRESTORE, and XSETBV/XGETBV.
        const OSXSAVE = 1 << 18;
...
impl XCr0 {
    ...
    ///
    /// # Safety
    /// This function is unsafe because it can trigger an #UD exception if
    /// the CPU does not support the `XGETBV` instruction or if this
    /// instruction use is not enabled in the CR4 register.
    #[must_use]
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing the XCR0 register can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing enabling XCR0 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing disabling XCR0 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
...
impl Cr0 {
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing the CR0 register can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing enabling CR0 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    pub unsafe fn enable(flags: Self) {
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing disabling CR0 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    pub unsafe fn disable(flags: Self) {
...
impl Cr3 {
    /// Read the current physical address of the page table (PML4) for
...
impl Cr4 {
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing the CR4 register can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing enabling CR4 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    pub unsafe fn enable(flags: Self) {
    ...
    ///
    /// # Safety
    /// This function is unsafe because changing disabling CR4 flags can,
    /// depending on the flags and on the current state of the CPU, break
    /// memory safety, cause undefined behavior or even reboot the computer
    /// if a triple fault occurs.
    pub unsafe fn disable(flags: Self) {
...
///
/// # Safety
/// This function is unsafe because it can trigger an #UD exception if the
/// CPU does not support the `XGETBV` instruction or if this instruction use
/// is not enabled in the CR4 register.
#[must_use]
...
/// # Safety
/// This function is unsafe because it can trigger an #UD exception if the
/// CPU does not support the `XSETBV` instruction or if this instruction use
/// is not enabled in the CR4 register. Furthermore, changing the XCR0 register
/// can, depending on the flags and on the current state of the CPU, break
/// memory safety, cause undefined behavior or even reboot the computer if a
/// triple fault occurs.
#[allow(clippy::cast_possible_truncation)]
...
///
/// # Safety
/// This function is unsafe because it deals with very low level stuff that
/// needs the use of unsafe code to work properly.
pub unsafe fn write_cr0(cr0: u64) {
    core::arch::asm!("mov cr0, {}", in(reg) cr0);
}

/// Read the current value of the CR2 register. This register contains the
...
/// Read the current value of the CR3 register. This register contains the
/// physical address of the page table (PML4) for the current CPU core.
#[must_use]
pub fn read_cr3() -> u64 {
    let cr3: u64;
    unsafe {
...
/// Write the given value to the CR3 register and change the current page table
/// (PML4) for the current CPU core. It will also flush all the TLB entries for
/// the current CPU core, except the global ones.
///
/// # Safety
/// This function is unsafe because it can break memory safety if the page
/// table is not properly constructed.
...
/// Wait for an interrupt to be triggered. This function is used to wait for
/// an interrupt when there is nothing else to do. It will enable interrupts,
/// halt the CPU and wait for an interrupt to be triggered. When an interrupt
/// is triggered, it will disable interrupts and return.
///
/// # Safety
/// This function is unsafe because the caller could be interrupted by an
/// interrupt handler, so the caller must take precautions to make this code
/// preemptable (by example, by not locking a mutex before calling this
/// function).
