...
/// was in progress, then we can try to kill the process because it is
/// likely that the fault was caused by the user process who tried to access
/// invalid memory or gave an invalid pointer to the kernel. If no user
/// operation was in progress, then we can't do anything and we must panic.
#[per_cpu]
static USER_OPERATION: Cell<bool> = Cell::new(false);

/// Checks if the current CPU is currently performing a user operation. This
/// function is used when a page fault occurs to know if the fault was caused
/// by a kernel performing a user operation or not. If the fault was caused by
/// a user operation, then we can try to fix it by killing the process. If the
/// fault was caused by the kernel, then we can't do anything and we must
/// panic.
#[must_use]
...
/// Write the given value to the given address. This function is implemented
/// by an simple call to [`copy_from`] with the same source and destination
...
/// Write the given value to the given address. This function is implemented
/// by an simple call to [`copy_to`] with the same source and destination
/// address and a length of 1. This will copy one `T` from the kernel to the
/// userland memory.
///
/// # Safety
/// This function is unsafe because it dereferences a user raw pointer that
...
/// that the pointer is valid and does not overlap with kernel space. However,
/// the caller does not need to ensure that the memory is readable, as this
/// function will handle page faults and kill the
pub unsafe fn write<T>(src: *const T, dst: *mut T) {
    copy_to(src, dst, 1);
}

/// Read an user c-string from the given address. It will try to read all the
/// bytes from the given address until it finds a null byte. If an maximum
/// length is given, then it will read  at most this number of bytes: if the
/// string is longer than this number, then it will return `None`.
///
/// # Safety
/// This function is unsafe because it dereferences a raw pointer, what's
/// more is an user pointer.
///
/// # Panics
/// This function should not panic. If it does, then it's a bug in this
/// function code.
#[must_use]
...
/// Compute the length of an user c-string. It will read all the bytes from
/// the given address until it finds a null byte.
///
/// # Safety
/// This function is unsafe because it dereferences a user raw pointer that
/// could possibly be invalid: it is the caller's responsibility to ensure
...
pub unsafe fn cstr_length(src: *const u8) -> usize {
    perform_user_operation(|| {
        (0..).position(|i| src.add(i).read() == 0).unwrap()
    })
}
...
/// this function when the current CPU is already performing an user
/// operation).
