...
/// The Global Descriptor Table. The size of the GDT is fixed and is equal to
/// 6 + 2 * `MAX_CPU`. The first 5 entries are reserved for the NULL
/// descriptor, the kernel code, the kernel data, the user code and the user
/// data, and the remaining entries are used for the TSS of each CPU.
pub static GDT: Spinlock<Table<{ 8 + MAX_CPUS * 2 }>> =
    Spinlock::new(Table::empty());

/// A structure that represents a GDT table and its register. The table is an
/// array of entries, with a compile-time fixed size (maximum 8192 entries).
#[derive(Clone)]
...
impl<const N: usize> Table<N> {
    ...
    /// Set the GDT entry at the given index to the given descriptor. If the
    /// entry is already in use, it will be overwritten. If the index is out
    /// of bounds, the behavior is undefined.
    ///
    /// # Warning
    /// If you set a system descriptor (i.e. a TSS descriptor), remember that
    /// it requires two GDT entries ! If you want to add a descriptor after a
    /// system descriptor, you need increment the index by 2.
    ///
    /// # Panics
    /// This function panics if the index is out of bounds (i.e. greater than
    /// or equal to the GDT's capacity) or if the entry is already in use.
    ...
    /// # Panics
    /// This function panics if the index is out of bounds (i.e. greater than
    /// or equal to the GDT's capacity)
    ...
    ///
    /// # Safety
    /// This function is unsafe because it can crash the kernel if the GDT is
...
impl Descriptor {
    ...
    pub fn tss(tss: &TaskStateSegment) -> Self {
        ...
        // Set the limit to the size of the TSS minus 1 (because the limit
        // is inclusive)
        low.set_bit_range(
            15,
            0,
...
impl Selector {
    ...
    /// Create a new segment selector. The index is the index of the segment
    /// in the GDT, and the privilege is the privilege level used for this
    /// segment.
    #[must_use]
...
/// Setup the GDT for the current CPU, load it and set the different segment
/// selectors registers.
///
/// # Safety
/// This function is unsafe because it can crash the kernel if the GDT is not
/// properly configured, or if the selectors are not properly set.
#[init]
pub unsafe fn setup() {
...
/// Load the GDT into the current CPU, and set the segment registers to their
/// default values.
///
/// # Safety
/// This function is unsafe because it can crash the kernel and/or break the
/// memory safety if the GDT is not properly configured before calling this
/// function.
#[init]
pub unsafe fn load() {
    GDT.lock().flush();
    ...
    unsafe {
        // Some black magic to load a new code segment selector. This is a
        // bit tricky because we cant directly load the new selector into
        // the CS register, and far jumps are not allowed in 64 bits mode.
        // So we use the 'retfq' instruction to set a new code segment selector
        core::arch::asm!(
            "mov ss, {1:r}",
            "mov ds, {1:r}",
            "mov es, {1:r}",
