...
/// base + 7] and the slave PIC will use IRQs [base + 8, base + 15].
/// After remapping, all interrupts are unmasked, but no interrupts will occur
/// until the interrupts are enabled with the `sti` instruction.
///
/// # Safety
/// This function is unsafe because it writes to the PICs with I/O ports,
/// which can cause undefined behavior if the PICs do not exist or are not
...
/// Check if the given IRQ number is in the range of the PICs. This is useful
/// for checking if an interrupt handler should send an EOI to the PICs.
...
/// Send an end-of-interrupt (EOI) to the PICs. This must be called after an
/// interrupt handler finishes executing. If the IRQ number is not in the range
/// of the PICs, this function does nothing.
///
/// The IRQ number needed as an argument is the IRQ number that the PICs are
/// configured to use, not the IRQ number that the CPU uses. For example, if
/// the PICs are configured to use an base IRQ of 32, then the IRQ number
/// needed as an argument is 0 if the CPU raised IRQ 32, 1 if the CPU raised
/// IRQ 33, etc.
///
/// # Safety
/// This function is unsafe because it writes to the PICs with I/O ports,
/// which can cause undefined behavior if the PICs do not exist or are not
/// in the expected state, or if it is used incorrectly.
...
/// Unmask all interrupts on the PICs. This is the default state after
/// remapping the PICs.
///
/// # Safety
/// This function is unsafe because it writes to the PICs with I/O ports,
/// which can cause undefined behavior if the PICs do not exist or are not
/// in the expected state, or if the PICs raise an interrupt that is not
/// handled by the IDT.
...
/// Mask all interrupts on the PICs. An interrupt masked by the PICs will
/// never occur and will not be sent to the CPU (lost forever).
///
/// # Safety
/// This function is unsafe because it writes to the PICs with I/O ports,
/// which can cause undefined behavior if the PICs do not exist or are not
