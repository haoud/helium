...
/// and can lead to a triple fault and a computer reboot if the interrupts are
...
/// Improve the CPU performance of spinlock loops. The processor uses this
...
/// Load the given GDT register into the CPU. The parameter is a pointer to
/// the GDT register.
///
/// # Safety
/// This function is unsafe because it can cause undefined behavior if the
/// given gdtr is not a valid GDT register, if the GDT is not loaded or not
/// properly configured...
#[inline(always)]
...
/// Load the given IDT register into the CPU. The parameter is a pointer to
/// the IDT register.
///
/// # Safety
/// This function is unsafe because it can cause undefined behavior if the
/// given idtr is not a valid IDT register, if the IDT is not loaded or not
/// properly configured...
#[inline(always)]
...
/// Load a new task state segment (TSS) into the CPU. The parameter is the
/// selector of the TSS.
///
/// # Safety
/// This function is unsafe because it can cause undefined behavior if the
/// given selector is not a valid TSS selector, if the TSS is not loaded or
/// not properly configured or if the GDT is not loaded or not properly
/// configured.
#[inline(always)]
...
/// Invalidate the TLB entry for the given virtual address. This is useful
/// when modifying the page tables, but should be used with caution because
/// it can dramatically reduce performances if used too often.
#[inline(always)]
