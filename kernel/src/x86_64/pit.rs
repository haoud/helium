...
/// The number of tick elapsed since the boot of the kernel. This variable is
/// incremented every time the PIT generates an IRQ.
...
///
/// # Safety
/// This function is unsafe because it use I/O ports to communicate with the
/// PIT, and might cause undefined behavior or memory safety issues if used
/// incorrectly.
#[init]
pub unsafe fn setup() {
...
/// Returns the elapsed time since the last IRQ in nanoseconds. In order to do
/// that, it reads the current value of the counter and calculates the elapsed
/// time since the last IRQ. Since this function read through the PIT and I/O
/// ports, it is not very fast, and should not be called often.
#[must_use]
